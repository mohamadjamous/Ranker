...
import { AddNominationFields, AddParticipantData, CreatePollFields, JoinPollFields, RejoinPollFields, SubmitRankingsFields } from './types';
...
export class PollsService {
  ...
  async createPoll(fields: CreatePollFields) {
    ...
    this.logger.debug(
      `Creating token string for pollID: ${pollID} and userID: ${userID}`
    )
  ...
  async joinPoll(fields: JoinPollFields) {
    ...
    this.logger.log(`Fetching poll with ID: ${fields.pollID} for user with ID: ${userID}`);
  ...
  async rejoinPoll(fields: RejoinPollFields) {
    ...
    this.logger.log(`Rejoining poll with pollID: ${fields.pollID} for user with ID: ${fields.userID} with name: ${fields.name}`);
